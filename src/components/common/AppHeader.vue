...
<template>
  ...
  >
    ...
    <div class="app-header__container container">
      ...
      <RouterLink to="/" class="app-header__brand" aria-label="Inicio - Xarxa Alimenta">
        <span class="app-header__logo" aria-hidden="true">
          <svg viewBox="0 0 40 40" width="40" height="40" fill="none" xmlns="http://www.w3.org/2000/svg">
            <circle cx="20" cy="20" r="19" fill="#ffffff" stroke="#216928" stroke-width="2" />
            <!-- Campo de cultivo en perspectiva y sol naciente -->
            <path d="M20 10 A 5 5 0 0 1 25 15 L 15 15 A 5 5 0 0 1 20 10 Z" fill="#e8a838" />
            <path d="M7 26 C13 23, 20 28, 33 24 L35 34 C24 37, 14 32, 5 35 Z" fill="#2e9e3a" />
            <path d="M10 21 C15 19, 21 23, 30 20 L32 26 C23 29, 16 25, 8 27 Z" fill="#216928" />
            <path d="M20 15 L20 22 M16 18 L24 18" stroke="#164a1b" stroke-width="1.5" stroke-linecap="round" />
          </svg>
        </span>
      ...
      <nav class="app-header__nav" :aria-label="t('nav.mainNavigation')">
        <ul class="app-header__nav-list">
          ...
          >
            ...
            >
              <span v-if="route.path === item.path" class="app-header__active-dot" aria-hidden="true" />
              {{ t(item.nameKey) }}
            </RouterLink>
          </li>
        </ul>
...
.app-header {
  ...
  border-block-end: 1.5px solid color-mix(in srgb, var(--color-border) 60%, transparent);
...
.app-header::before {
  ...
  right: 0;
  height: 4px;
  background: linear-gradient(90deg, var(--color-primary) 0%, var(--color-sun, #eab308) 50%, var(--color-accent) 100%);
...
.app-header--scrolled {
  ...
  box-shadow: 0 8px 30px -4px var(--color-glow, rgba(0, 0, 0, 0.12)), 0 2px 6px rgba(0, 0, 0, 0.04);
}

[data-theme="dark"] .app-header {
  background: color-mix(in srgb, var(--color-surface) 85%, transparent);
  border-block-end-color: color-mix(in srgb, var(--color-border) 80%, transparent);
}
